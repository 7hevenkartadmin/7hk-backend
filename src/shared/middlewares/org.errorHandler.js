...
const productionMessages = {
  ...
  PAYMENT_INTENT_MISMATCH:
    "Payment does not match this order. Please restart checkout.",
...
function publicMessage(error, statusCode, code) {
  if (env.NODE_ENV !== "production")
    return (
      error.message ||
      productionMessages[code] ||
      productionMessages.INTERNAL_ERROR
    );
...
export function errorHandler(error, req, res, _next) {
  ...
  const requestId =
    req.headers["x-request-id"] || crypto.randomUUID?.() || `${Date.now()}`;
  const payload = {
    success: false,
