...
orderRoutes.post(
  "/quote",
  validate(quoteOrderSchema),
  asyncHandler(async (req, res) => {
    ok(res, await quoteOrder(req.body), "Order quote calculated");
  }),
);
...
orderRoutes.get(
  ...
  asyncHandler(async (req, res) => {
    ok(
      res,
      { orders: await listCustomerOrders(req.user._id) },
      "Orders loaded",
    );
  }),
);
...
orderRoutes.get(
  ...
  asyncHandler(async (req, res) => {
    ok(
      res,
      { order: await getOrderForCustomer(req.params.idOrNumber, req.user) },
      "Order loaded",
    );
  }),
);
...
orderRoutes.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    ok(res, { invoice: order.invoice, order }, "Invoice loaded");
  }),
);
