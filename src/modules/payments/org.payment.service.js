...
import {
  calculateCartTotals,
  defaultDeliveryFee,
} from "../orders/pricing.service.js";
...
export function createCartHash(items, couponCode, total) {
  const normalized = {
...
export async function createRazorpayCheckoutSession(payload, customer) {
  ...
  const providerOrder = await client.orders.create({
    ...
    notes: { source: "web_checkout", userId: String(customer._id) },
  });
...
export async function createCapturedRazorpayPaymentForOrder(
  order,
  payload,
  session,
) {
...
export async function consumeRazorpayPaymentIntent({
  customer,
  payload,
  items,
  couponCode,
  ...
  verifyRazorpaySignature(payload);
  const cartHash = createCartHash(items, couponCode, total);
  const update = {
    status: "consumed",
  ...
  if (!intent)
    throw new AppError(
      "Payment session does not match this order",
      409,
